...
export const CatalogoBusqueda = () => {
  ...
  const ObtenerTituloFiltrado = () => {
    ...
    const titulosGuardados = JSON.parse(
      localStorage.getItem("BusquedaDeItemsGameRealm") ?? "[]"
    );
      ...
      {titulos.map((titulos: any) => (
        <Link to={`./Orden?codigo=${titulos.codigo}`} key={titulos.codigo}>
          <article className="card-contenido contenido-producto">
            <div>
              <img
                className="ajustar-imagen imagen-recortada"
                src={titulos.imagen}
                alt="..."
              />
              ...
              <p>
                Precio: <span className="span-precio">${titulos.precio}</span>
              </p>

              <p>
                {titulos.descripcion.substring(0, 50)}{" "}
              ...
              <input
                type="submit"
                value="Seleccionar"
                className="boton boton-amarillo boton-largo ver-mas"
              />
            </div>
          </article>
        </Link>
